...
export async function measureLogo(source) {
  ...
  const fallback = { aspect: native ? native.width / native.height : REF_ASPECT, rect: null }
      ...
      if (res.ok) {
        ...
        if (box) {
          ...
          if (width > 8 && height > 4 && area > 0.01 && area < 0.995) {
            result = { aspect: width / height, rect: { left, top, width, height } }
          }
        }
      }
    }
...
export function logoSrc(source, measurement, renderWidth) {
  ...
  return builder
    .width(Math.round(renderWidth * 3))
    .fit('max')
    .auto('format')
    .url()
}
